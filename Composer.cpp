...
void compose(char* front, size_t f_width, size_t f_height, char* back, size_t b_width, size_t b_height){
	...
	for (size_t y = 0; y < f_height; ++y){
		for (size_t x = 0; x < f_width; x += 4){
			...
			__m128i bp   = _mm_lddqu_si128(reinterpret_cast<__m128i*>(back  + x * 4));
			__m128i fp   = _mm_lddqu_si128(reinterpret_cast<__m128i*>(front + x * 4));
...
void slow_compose(char* front, size_t f_width, size_t f_height, char* back, size_t b_width, size_t b_height){
	...
	for (size_t y = 0; y < f_height; ++y){
		for (size_t x = 0; x < f_width; x += 1){
			...
			uint8_t alpha = *(uint8_t*)(front + 4 * x + 3);
			uint8_t inv_alpha = 255 - alpha;

			back[4 * x]     = (uint8_t)((alpha * front[4 * x]     + inv_alpha * back[4 * x])     >> 8);
			back[4 * x + 1] = (uint8_t)((alpha * front[4 * x + 1] + inv_alpha * back[4 * x + 1]) >> 8);
			back[4 * x + 2] = (uint8_t)((alpha * front[4 * x + 2] + inv_alpha * back[4 * x + 2]) >> 8);
